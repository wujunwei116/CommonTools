            ...
            private struct DeserializerKey : IEquatable<DeserializerKey>
            {
                ...
                public bool Equals(DeserializerKey other)
                {
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        if (GetName(i) != other.GetName(i)||GetType(i)!=other.GetType(i))
                        {
                            return false;
                        }
                ...
                {
                    if (reader != null )
                        return names[i] != null ? names[i] : reader.GetName(startBound + 1);
                    else
                        return names[i] != null ? names[i] : null;
                }

                public Type GetType(int i)
                {
                    if (reader != null)
                        return types[i] != null ? types[i] : reader.GetFieldType(startBound + 1);
                    else
                        return types[i] != null ? types[i] : null;
                }
